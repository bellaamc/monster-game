...
/**
 * 
 * AssignEdibleToMonster allows a monster to use an edible
 *
 */
public class AssignEdibleToMonster {

	/** setups the basic frame */
	...
	public static void main(GameEnvironment newGame) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AssignEdibleToMonster window = new AssignEdibleToMonster(newGame);
	...
	/**
	 * AssignEdibleToMonster Create the application.
	 * @param newGame inputs the gameEnvironment
	 * 
	 */
	public AssignEdibleToMonster(GameEnvironment newGame) {
		initialize(newGame);
	}
		...
		if (newGame.getMonsterInventory().size() > 0) { 
			...
			monster1.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					Item edible = newGame.getSelectedEdible() ;
					newGame.useItem(edible, newGame.getMonsterInventory().get(0));
			...
			if (newGame.getMonsterInventory().size() > 1) {
				...
				monster2.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						Item edible = newGame.getSelectedEdible() ;
						newGame.useItem(edible, newGame.getMonsterInventory().get(1));
						EdiblesInventory.main(newGame);
						frame.dispose(); 
				}
				});
				...
				if (newGame.getMonsterInventory().size() > 2) {
					...
					monster3.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							Item edible = newGame.getSelectedEdible() ;
							newGame.useItem(edible, newGame.getMonsterInventory().get(2));
							EdiblesInventory.main(newGame);
							frame.dispose(); 
					}
					});
					...
					if (newGame.getMonsterInventory().size() > 3) {
						...
						monster4.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								Item edible = newGame.getSelectedEdible() ;
								newGame.useItem(edible, newGame.getMonsterInventory().get(3));
								EdiblesInventory.main(newGame);
								frame.dispose(); 
						}
						});
